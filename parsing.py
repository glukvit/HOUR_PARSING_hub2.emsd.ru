...
needtime = curtime.strftime('%Y%m%d%H') #Дата которую потом переведем для записи в df_fin[''DECDATE']
tosec = datetime.datetime.strptime(needtime,"%Y%m%d%H")#Переводим стринг в дейттайм
dctm = int(tosec.timestamp())#время в секундах
...
df_new_line.loc[0,'DECDATE'] = dctm
cols = ['DATE','DECDATE','HAE','HAN','HK2']
df_new_line = df_new_line[cols]
...
if os.path.exists(path): # Проверяем наличие двухнедельного файла 
    ...
    else: #Если в окончательном файле меньше 336 значений, то просто добавляем новые данные
        df_fin=pd.concat([df_fin,df_new_line]) # Добавляем новые данные из df_new_line в записываемый в файл df_fin.
else:
    df_fin = df_new_line # Если файла, который создавался нет, то просто запишем имеющиеся данные в файл.

df_fin=df_fin.drop_duplicates(subset = ['DECDATE'],keep = False) #Если есть дубликаты в строках удаляем. Проверка дубликатов по столбцу 'DATE'
